...
@Composable
fun ActiveDownloadItem(item: MangaDownloadItem, onCancel: () -> Unit) {
    ListItem(
        headlineContent = { Text(text = item.download.chapter.name) },
        supportingContent = { Text(text = item.download.manga.title) },
        trailingContent = {
    ...
    Scaffold {
        Column(modifier = Modifier.padding(contentPadding)) {
            accordionState.activeItem?.let {
                ActiveDownloadItem(item = it, onCancel = { screenModel.cancel(listOf(it.download)) })
            }
