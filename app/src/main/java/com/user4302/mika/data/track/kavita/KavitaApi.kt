package com.user4302.mika.data.track.kavita
...
class KavitaApi(private val client: OkHttpClient, interceptor: KavitaInterceptor) {
    ...
    fun getNewToken(apiUrl: String, apiKey: String): String? {
        val request = POST(
            "$apiUrl/Plugin/authenticate?apiKey=$apiKey&pluginName=MiKa-Kavita",
    ...
    private fun getLatestChapterRead(url: String): Double {
        ...
        val requestUrl = "${getApiFromUrl(url)}/MiKa/latest-chapter?seriesId=$seriesId"
        try {
            with(json) {
    ...
    suspend fun updateProgress(track: MangaTrack): MangaTrack {
        val requestUrl = "${getApiFromUrl(
            track.tracking_url,
        )}/MiKa/mark-chapter-until-as-read?seriesId=${getIdFromUrl(
            track.tracking_url,
