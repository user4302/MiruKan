...
import com.user4302.mika.decoder.ImageDecoder
import okio.BufferedSource

/**
 * A [Decoder] that uses built-in [ImageDecoder] to decode images that is not supported by the system.
 */
class MikaImageDecoder(private val resources: ImageSource, private val options: Options) : Decoder {
    ...
    class Factory : Decoder.Factory {
        ...
        override fun create(result: SourceFetchResult, options: Options, imageLoader: ImageLoader): Decoder? {
            return if (options.customDecoder || isApplicable(result.source.source())) {
                MikaImageDecoder(result.source, options)
            } else {
                null
            }
        }
