...
class KomgaApi(
    ...
    private val headers: Headers by lazy {
        Headers.Builder()
            .add("User-Agent", "MiKa v${BuildConfig.VERSION_NAME} (${BuildConfig.APPLICATION_ID})")
            .build()
    }
    ...
    suspend fun getTrackSearch(url: String): MangaTrackSearch =
        withIOContext {
            try {
                ...
                val progress = client
                    .newCall(
                        GET("${url.replace("/api/v1/series/", "/api/v2/series/")}/read-progress/mika", headers),
                    )
                    .awaitSuccess().let {
                        with(json) {
    ...
    suspend fun updateProgress(track: MangaTrack): MangaTrack {
        ...
        client.newCall(
            Request.Builder()
                .url(
                    "${track.tracking_url.replace("/api/v1/series/", "/api/v2/series/")}/read-progress/mika",
                )
                .headers(headers)
