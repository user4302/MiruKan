...
import com.user4302.mika.ui.reader.viewer.webtoon.WebtoonViewer
import java.text.NumberFormat

@Composable
internal fun ColumnScope.ReadingModePage(screenModel: ReaderSettingsScreenModel) {
    HeadingItem(AYMR.strings.pref_category_for_this_series)
    ...
    SettingsChipRow(AYMR.strings.pref_category_reading_mode) {
    ...
    SettingsChipRow(AYMR.strings.rotation_type) {
...
private fun ColumnScope.PagerViewerSettings(screenModel: ReaderSettingsScreenModel) {
    HeadingItem(AYMR.strings.pager_viewer)
    ...
    SettingsChipRow(AYMR.strings.pref_image_scale_type) {
    ...
    SettingsChipRow(AYMR.strings.pref_zoom_start) {
    ...
    CheckboxItem(
        label = stringResource(AYMR.strings.pref_crop_borders),
        pref = screenModel.preferences.cropBorders(),
    )

    CheckboxItem(
        label = stringResource(AYMR.strings.pref_landscape_zoom),
        pref = screenModel.preferences.landscapeZoom(),
    )

    CheckboxItem(
        label = stringResource(AYMR.strings.pref_navigate_pan),
        pref = screenModel.preferences.navigateToPan(),
    )

    val dualPageSplitPaged by screenModel.preferences.dualPageSplitPaged().collectAsState()
    CheckboxItem(
        label = stringResource(AYMR.strings.pref_dual_page_split),
        pref = screenModel.preferences.dualPageInvertPaged(),
    )

    if (dualPageSplitPaged) {
        CheckboxItem(
            label = stringResource(AYMR.strings.pref_dual_page_invert),
    ...
    CheckboxItem(
        label = stringResource(AYMR.strings.pref_page_rotate),
        pref = screenModel.preferences.dualPageRotateToFit(),
    )

    if (dualPageRotateToFit) {
        CheckboxItem(
            label = stringResource(AYMR.strings.pref_page_rotate_invert),
            pref = screenModel.preferences.dualPageRotateToFitInvert(),
        )
    }
}
...
private fun ColumnScope.WebtoonViewerSettings(screenModel: ReaderSettingsScreenModel) {
    ...
    HeadingItem(AYMR.strings.webtoon_viewer)
    ...
    SliderItem(
        ...
        label = stringResource(AYMR.strings.pref_webtoon_side_padding),
    ...
    CheckboxItem(
        label = stringResource(AYMR.strings.pref_double_tap_zoom),
        pref = screenModel.preferences.webtoonDoubleTapZoomEnabled(),
    )
    CheckboxItem(
        label = stringResource(AYMR.strings.pref_webtoon_disable_zoom_out),
...
private fun ColumnScope.TapZonesItems(
    ...
    SettingsChipRow(AYMR.strings.pref_viewer_nav) {
    ...
    if (selected != 5) {
        SettingsChipRow(AYMR.strings.pref_read_with_tapping_inverted) {
