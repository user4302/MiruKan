...
import com.user4302.mika.data.backup.restore.RestoreOptions
...
class RestoreBackupScreen(
    ...
    override fun Content() {
        ...
        Scaffold(
            topBar = {
                AppBar(
                    title = stringResource(AYMR.strings.pref_restore_backup),
            ...
            LazyColumnWithAction(
                ...
                actionLabel = stringResource(AYMR.strings.action_restore),
                    ...
                    val msg = buildAnnotatedString {
                        when (error) {
                            is MissingRestoreComponents -> {
                                appendLine(stringResource(AYMR.strings.backup_restore_content_full))
                                if (error.sources.isNotEmpty()) {
                                    appendLine()
                                    withStyle(SpanStyle(fontWeight = FontWeight.Bold)) {
                                        appendLine(stringResource(AYMR.strings.backup_restore_missing_sources))
                                    }
                                ...
                                if (error.trackers.isNotEmpty()) {
                                    appendLine()
                                    withStyle(SpanStyle(fontWeight = FontWeight.Bold)) {
                                        appendLine(stringResource(AYMR.strings.backup_restore_missing_trackers))
                                    }
                            ...
                            is InvalidRestore -> {
                                withStyle(SpanStyle(fontWeight = FontWeight.Bold)) {
                                    appendLine(stringResource(AYMR.strings.invalid_backup_file))
                                }
                                ...
                                withStyle(SpanStyle(fontWeight = FontWeight.Bold)) {
                                    appendLine(stringResource(AYMR.strings.invalid_backup_file_error))
                                }
