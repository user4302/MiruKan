...
import androidx.compose.ui.focus.focusRequester
import com.user4302.core.preference.asToggleableState
...
fun CategoryCreateDialog(
    ...
    AlertDialog(
        ...
        title = {
            Text(text = stringResource(AYMR.strings.action_add_category))
        },
        text = {
            OutlinedTextField(
                modifier = Modifier
                ...
                label = {
                    Text(text = stringResource(AYMR.strings.name))
                },
                supportingText = {
                    val msgRes = if (name.isNotEmpty() && nameAlreadyExists) {
                        AYMR.strings.error_category_exists
                    } else {
...
fun CategoryRenameDialog(
    ...
    AlertDialog(
        ...
        title = {
            Text(text = stringResource(AYMR.strings.action_rename_category))
        },
        text = {
            OutlinedTextField(
                ...
                label = { Text(text = stringResource(AYMR.strings.name)) },
                supportingText = {
...
fun CategoryDeleteDialog(
    ...
    AlertDialog(
        ...
        title = {
            Text(text = stringResource(AYMR.strings.delete_category))
        },
        text = {
            Text(text = stringResource(AYMR.strings.delete_category_confirmation, category))
        },
    )
}
...
fun ChangeCategoryDialog(
    ...
    if (initialSelection.isEmpty()) {
        AlertDialog(
            onDismissRequest = onDismissRequest,
            confirmButton = {
                mika.presentation.core.components.material.TextButton(
                    onClick = {
                        onDismissRequest()
                        onEditCategories()
                    },
                ) {
                    Text(text = stringResource(AYMR.strings.action_edit_categories))
                }
            },
            title = {
                Text(text = stringResource(AYMR.strings.action_move_category))
            },
            text = {
                Text(text = stringResource(AYMR.strings.information_empty_category_dialog))
            },
        )
        return
    }
    ...
    AlertDialog(
        ...
        confirmButton = {
            Row {
                mika.presentation.core.components.material.TextButton(onClick = {
                    onDismissRequest()
                    onEditCategories()
                }) {
                    Text(text = stringResource(AYMR.strings.action_edit))
                }
                Spacer(modifier = Modifier.weight(1f))
                mika.presentation.core.components.material.TextButton(onClick = onDismissRequest) {
                    Text(text = stringResource(AYMR.strings.action_cancel))
                }
                mika.presentation.core.components.material.TextButton(
                    onClick = {
