...
import com.user4302.mika.domain.items.chapter.service.calculateChapterGap
...
import com.user4302.mika.ui.reader.model.ChapterTransition
    ...
    ProvideTextStyle(MaterialTheme.typography.bodyMedium) {
        when (transition) {
            is ChapterTransition.Prev -> {
                TransitionText(
                    topLabel = stringResource(AYMR.strings.transition_previous),
                    topChapter = goingToChapter,
                    topChapterDownloaded = goingToChapterDownloaded,
                    bottomLabel = stringResource(AYMR.strings.transition_current),
                    bottomChapter = currChapter,
                    bottomChapterDownloaded = currChapterDownloaded,
                    fallbackLabel = stringResource(AYMR.strings.transition_no_previous),
                    chapterGap = calculateChapterGap(currChapter, goingToChapter),
                )
            }
            is ChapterTransition.Next -> {
                TransitionText(
                    topLabel = stringResource(AYMR.strings.transition_finished),
                    topChapter = currChapter,
                    topChapterDownloaded = currChapterDownloaded,
                    bottomLabel = stringResource(AYMR.strings.transition_next),
                    bottomChapter = goingToChapter,
                    bottomChapterDownloaded = goingToChapterDownloaded,
                    fallbackLabel = stringResource(AYMR.strings.transition_no_next),
        ...
        Row(
            ...
            Text(
                text = pluralStringResource(
                    AYMR.plurals.missing_chapters_warning,
                    count = gapCount,
                    gapCount,
                ),
                style = MaterialTheme.typography.bodyMedium,
            ...
            inlineContent = persistentMapOf(
                DOWNLOADED_ICON_ID to InlineTextContent(
                    ...
                    Icon(
                        ...
                        contentDescription = stringResource(AYMR.strings.label_downloaded),
                    )
                },
            ),
        )
